...
features: Dict[int, Dict[str, int]] = {}  # platform_id:[name: id]
...
def get_feature_id(feature_name: str, platform_id: int) -> int:
    global features
    if platform_id not in features:
        features[platform_id] = {}
    if feature_name not in features[platform_id]:
        from lib.db_api import get_feature_id as get_feature_id_db
        features[platform_id][feature_name] = get_feature_id_db(feature_name, platform_id)
    return features[platform_id][feature_name]
